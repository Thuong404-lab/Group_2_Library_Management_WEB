...
colors:
  ...
  warning: "#d4a017"
  error: "#c64545"
  application-primary: "#8b5a2b"
  application-primary-hover: "#6b4423"
  application-primary-active: "#58371d"
  application-primary-disabled: "#c9b8a7"
  focus-coral-ring: "rgba(204,120,92,0.20)"
  focus-application-ring: "rgba(139,90,43,0.22)"
  success-soft: "#e7f4ea"
  success-border: "#bddfc5"
  success-ink: "#2f6f3e"
  warning-soft: "#fbf3d9"
  warning-border: "#eadfb8"
  warning-ink: "#795d12"
  danger-soft: "#f8e5e2"
  ...
  info-border: "#c6dadd"
  info-ink: "#355f66"
  overlay-backdrop: "rgba(20,20,19,0.56)"
  overlay-shadow: "0 18px 48px rgba(20,20,19,0.18)"
...
components:
  ...
  app-icon-action-delete:
    ...
    size: 36px
  app-primary-action:
    backgroundColor: "{colors.application-primary}"
    textColor: "{colors.on-dark}"
    typography: "{typography.button}"
    rounded: "{rounded.md}"
    padding: 0 18px
...
- **Application Primary** (`{colors.application-primary}` — #8b5a2b): Canonical authenticated-application create-action brown, taken from Admin Book Inventory. Hover is `{colors.application-primary-hover}` (#6b4423), pressed is `{colors.application-primary-active}` (#58371d), and disabled is `{colors.application-primary-disabled}` (#c9b8a7). Do not substitute coral or a role-specific color for the canonical “Thêm/Tạo mới” action.
...
### Application Semantic Color Contract

| Meaning | Strong/accent | Soft surface | Border | Text/icon |
|---|---|---|---|---|
| Success / available / completed | `{colors.success}` | `{colors.success-soft}` | `{colors.success-border}` | `{colors.success-ink}` |
| Warning / pending / attention | `{colors.warning}` | `{colors.warning-soft}` | `{colors.warning-border}` | `{colors.warning-ink}` |
| Danger / error / destructive | `{colors.error}` | `{colors.danger-soft}` | `{colors.danger-border}` | `{colors.danger-ink}` |
| Info / neutral guidance | `{colors.info-ink}` | `{colors.info-soft}` | `{colors.info-border}` | `{colors.info-ink}` |

Use semantic colors only for their named meaning. Status badges, alerts, validation, toasts, and destructive actions share this mapping across Admin, Librarian, Member, and Guest. Categories and roles do not receive semantic colors for decoration. On light surfaces, normal semantic text uses the `*-ink` token and the background uses the matching `*-soft`; strong/accent colors are reserved for a leading accent, small icon, progress indicator, or control whose contract explicitly requires it.

The default hierarchy is fixed: page canvas `{colors.canvas}`, secondary region `{colors.surface-soft}`, card/table/modal surface `{colors.canvas}`, subdued card `{colors.surface-card}`, border `{colors.hairline}`, primary text `{colors.ink}`, body `{colors.body}`, secondary text `{colors.muted}`. Do not introduce pure white, cool gray, Bootstrap blue, role-specific palettes, or arbitrary opacity as alternate surfaces.

All text and interactive colors must meet WCAG contrast: 4.5:1 for normal text and 3:1 for large text, icons, component boundaries, and focus indicators. Color never carries meaning alone; pair it with a label and the canonical icon where applicable.

### Mandatory Color Assignment Matrix

This matrix is the canonical mapping for all pages and roles. The same visual role always consumes the same token.

| UI role | Required token(s) | Forbidden substitutions |
|---|---|---|
| Page background | `{colors.canvas}` | pure white, cool gray, role-specific background |
| Secondary section/hover surface | `{colors.surface-soft}` | arbitrary alpha tint, Bootstrap light |
| Card/table/modal/dropdown surface | `{colors.canvas}` | page-specific white/cream |
| Subdued/selected card surface | `{colors.surface-card}` | semantic color used decoratively |
| Primary heading/icon | `{colors.ink}` | pure black, role color |
| Body text | `{colors.body}` | Bootstrap body gray, inline gray |
| Emphasized body text | `{colors.body-strong}` | arbitrary dark brown/black |
| Secondary/helper/breadcrumb | `{colors.muted}` | opacity applied to body text |
| Disabled/caption text | `{colors.muted-soft}` | arbitrary low-opacity text |
| Standard border | `{colors.hairline}` | Bootstrap border gray |
| Internal divider | `{colors.hairline-soft}` | copied border with opacity |
| Public/marketing primary CTA | `{colors.primary}` / `{colors.primary-active}` | application brown or Bootstrap blue |
| Authenticated create/confirm action | `{colors.application-primary}` / hover/active/disabled tokens | coral, role-specific or domain-specific primary |
| Standard focus ring | `{colors.focus-coral-ring}` | browser blue or arbitrary shadow |
| Application-primary focus ring | `{colors.focus-application-ring}` | opaque border or page-local ring |
| Destructive action/validation | danger token family | Bootstrap danger palette, solid red modal header |
| Success state | success token family | arbitrary green or teal |
| Warning/pending state | warning token family | arbitrary orange/yellow |
| Informational state | info token family | Bootstrap primary blue |
| Dark overlay/tooltip/footer | `{colors.surface-dark}` with `{colors.on-dark}` / `{colors.on-dark-soft}` | pure black/white pairing |
| Modal backdrop | `{colors.overlay-backdrop}` | arbitrary opacity or multiple backdrops |
| Overlay elevation | `{colors.overlay-shadow}` | page-specific shadow color |

### Token-Only Implementation Rule

The YAML `colors:` map above is the single color registry. Implementations reference it through the project's canonical shared CSS custom properties or canonical component classes. A repeated hex—even if visually identical to a token—is a duplicate color declaration and does not count as token reuse.

- Do not create page, role, feature, or contributor palettes. Names such as `admin-primary`, `librarian-brown`, `book-color`, `custom-danger`, and `new-theme` are forbidden aliases.
- Do not generate undocumented shades with opacity, filters, gradients, `color-mix()`, or copied Bootstrap variables. Every default, hover, focus, active, selected, disabled, loading, semantic, overlay, SVG fill/stroke, and shadow color must resolve to a named token.
- `inherit`/`currentColor` are valid only inside a canonical component whose parent owns the required token. `transparent` is valid only where the component contract intentionally has no surface.
- User-uploaded images, book covers, official logos, and content media may retain intrinsic colors; application chrome around them may not borrow those colors.
- Charts require a separately documented chart-series palette before implementation. Do not use semantic state colors as arbitrary data-series colors.
- When a genuinely required color is absent, update this file first with one semantic token and its allowed usage. Never solve the gap by adding a literal to page CSS.

## Typography

### Font Family
The system runs **Copernicus** (or **Tiempos Headline** as substitute) as the slab-serif display face for headlines, and **StyreneB** (or **Inter** as substitute) as the humanist sans for body, navigation, and UI labels. **JetBrains Mono** handles code blocks. Use the complete display stack `Copernicus, "Tiempos Headline", "Playfair Display", "Cormorant Garamond", Garamond, serif` and UI stack `StyreneB, Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif`; do not shorten or reorder these stacks per page.
...
### Application Text-Role Contract

Authenticated Admin, Librarian, and Member pages use the same family for the same semantic role:

| Application area | Required family | Required style |
|---|---|---|
| Page title | Display serif | `clamp(30px, 3vw, 40px)`, 400/1.15, `-.02em` |
| Card/section/modal title | Display serif | 20px, 500/1.3 |
| Sidebar brand name | Display serif | 20px, 500/1.2 |
| Dashboard statistic value | Display serif | 28px, 500/1.15 |
| Navigation, sidebar item, tab/filter | UI sans | 14px, 500–600/1.4 according to the canonical component |
| Body, table cell, form control, modal body | UI sans | 14px, 400/1.5; form controls use 1.4 |
| Button | UI sans | 14px, 600/1; compact button is 13px/600/1 only |
| Table header | UI sans | 12px, 600/1.4, `.08em`, uppercase |
| Form label | UI sans | 14px, 600/1.4 |
| Breadcrumb, helper, secondary text | UI sans | 13px, 400–500/1.4 |
| Badge/status | UI sans | 12px, 600/1 |
| Tooltip and validation | UI sans | 12px, 500/1.4 |
| Code/terminal | Monospace | 14px, 400/1.6 |

Do not use display serif for navigation, buttons, form labels/controls, tables, badges, breadcrumbs, pagination, alerts, tooltips, or ordinary paragraphs. Do not use UI sans for canonical page/section/modal titles merely because a page is in another role. Hover, focus, active, selected, and disabled states never change typography or cause text reflow; only documented color, surface, border, decoration, or opacity may change.

Vietnamese labels must preserve diacritics and be tested with realistic long content. Never solve wrapping by shrinking canonical type. Use the layout/width owner, approved wrapping, or truncation contract instead. Synthetic bold and italic are forbidden; use a loaded documented weight or the approved fallback stack.
...
Typography is semantic, not element-driven: raw `h1`–`h6`, `<strong>`, `<small>`, Bootstrap `fs-*`/`fw-*`, and browser defaults do not define the visual role. Canonical component classes own family, size, weight, line-height, tracking, and casing so equivalent UI remains identical across pages and roles.
...
Every documented component is canonical: reuse its DOM structure, classes, sizing, spacing, states, interaction, and responsive behavior instead of creating a visually equivalent variant. Each page family has exactly one CSS visual owner, and equivalent Admin, Librarian, and Member components use the same visual contract; role differences may change content, data, permissions, routes, and available actions, but not presentation. HTML/Thymeleaf owns semantic structure and behavior hooks only—presentation belongs in the canonical external stylesheet, never in `<style>`, inline `style`, or static `th:style` declarations.
...
**`app-icon-action`** — Canonical authenticated-table action used for View and Edit. Exactly 36×36px with zero padding, 1px `{colors.hairline}` border, `{colors.canvas}` background, `{colors.ink}` icon, `{rounded.full}`, and a centered 16px icon. Hover uses `{colors.surface-soft}` background, `{colors.primary}` border, and `{colors.primary-active}` icon; focus-visible uses `{colors.focus-coral-ring}`. Active uses `{colors.surface-card}`. Interaction may lift at most 1px and never changes geometry.

**`app-icon-action-delete`** — Same geometry and states as `{component.app-icon-action}`, but its default icon is `{colors.error}`. The canonical three-action sequence is eye (`bi-eye`) → edit (`bi-pencil-square`) → delete (`bi-trash`) with an 8px gap; semantic color affects the icon only, never the circular surface size.

**`app-primary-action`** — Canonical page-level “Thêm/Tạo mới” action for authenticated management pages, locked to Admin Book Inventory. Background `{colors.application-primary}`, text `{colors.on-dark}`, exact height 40px, horizontal padding 18px, `{rounded.md}`, 14px/600, line-height 1. It sits at the far right of the section toolbar on desktop; hover/focus uses `{colors.application-primary-hover}`, pressed uses `{colors.application-primary-active}`, and focus-visible uses `{colors.focus-application-ring}`. Below 768px the toolbar stacks and the button becomes full-width while remaining 40px high.
...
### Modals, Popups, and Feedback

**`app-modal`** — Canonical shell for create/edit/detail/confirmation dialogs. The single `.modal-content` frame uses `{colors.canvas}`, 1px `{colors.hairline}`, `{rounded.lg}` (12px), and `{colors.overlay-shadow}`. Header and footer use `{colors.surface-soft}` with `{colors.hairline-soft}` dividers; body uses `{colors.canvas}`. Desktop padding is 20px header, 24px body, 20px footer; all become 16px on mobile. The backdrop is `{colors.overlay-backdrop}`.

Modal sizes are fixed variants: 400px small confirmation, 560px default form/detail, and 800px large data/detail, each capped at `calc(100vw - 32px)`. Only the body scrolls vertically. A solid semantic-color header is forbidden; semantic meaning belongs to a small icon/accent, copy, and the final action.

The header uses one 20px/500 display-serif title and one 36×36px close action. The footer is a right-aligned, non-wrapping row with 8px gap: secondary/cancel first, primary/confirm last. Destructive confirmation changes only the final action to the danger variant. Opening traps focus; closing restores focus to the trigger.

**`app-toast`** — Canvas feedback card with hairline border, `{rounded.lg}`, `{colors.overlay-shadow}`, 16px padding, and maximum width 360px. It uses a 4px leading semantic accent plus the matching icon; success/warning/danger/info never change toast geometry. Toasts stack top-right on desktop and use 16px left/right mobile insets.

**`app-tooltip`** — Short label only: `{colors.surface-dark}` background, `{colors.on-dark}` text, 12px/500, `{rounded.sm}`, 8px × 10px padding, and 240px maximum width. Brown/red/page-specific tooltip surfaces are forbidden. Essential instructions must remain visible elsewhere.

**`app-dropdown` / `app-popover`** — `{colors.canvas}` surface, hairline border, `{rounded.md}`, canonical overlay shadow, 8px padding, and 40px minimum item height. Hover uses `{colors.surface-soft}`. A destructive item uses `{colors.danger-ink}` text without turning the entire surface red.

**Overlay behavior** — Dropdowns/tooltips/popovers sit above page content; modal backdrop sits below modal content; toasts do not cover an active decision modal. Central shared CSS owns the z-index scale—page CSS never uses arbitrary `9999` values. Overlay transitions run 150–200ms ease and remove translation/scale under `prefers-reduced-motion: reduce`.
...
- Don't invent hover styling. Use only the canonical hover/focus/active/disabled states documented for the component; those states must not change component dimensions or surrounding layout.
...
4. Hover/focus may be documented only for an established canonical interactive component. Reuse the documented state exactly; never create a page-local interaction variant.
