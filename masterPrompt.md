...
Báo cáo cuối phải liệt kê tất cả file đã thay đổi và lý do thay đổi từng file. 3. Thứ tự ưu tiên
...
Không hy sinh nghiệp vụ đúng chỉ để code hoặc giao diện trông đẹp hơn. 4. Quy trình bắt buộc trước khi sửa
...
Không đoán method, field, endpoint hoặc nghiệp vụ chưa tồn tại. 5. Bảo toàn code và contract
...
ID, class và data-_ được JavaScript sử dụng.
aria-_ có ý nghĩa chức năng.
...
Không tạo file HTML hoặc CSS mới nếu trang đã có file tương ứng. Ưu tiên chỉnh sửa và tái sử dụng file hiện có. 6. Kiến trúc backend
Giữ cấu trúc:
Controller
→ Service
→ Repository
...
Webhook và thao tác tài chính phải idempotent. 7. Exception và validation
...
Frontend validation chỉ hỗ trợ trải nghiệm người dùng. 8. Security
Phải giữ:
...
Thao tác tài chính phải có unique reference và audit trail. 9. I18n Anh–Việt
...
Modal.
Navigation.
QUY CHUẨN UI/UX 10. Nguồn giao diện chuẩn
Sử dụng theo thứ tự:
Tổng thể
...
Không thêm UI framework mới. 12. Typography
...
Hai dãy trên phải sử dụng cùng typography và chiều cao header. 13. Banner và heading
...
Gửi đề xuất mới.
Tất cả thông báo.
Nếu ẩn heading hiển thị, vẫn giữ semantic heading hoặc accessible label khi cần. 14. List, table và card
Mỗi item phải có:
Đủ bốn góc bo.
Nền trắng.
Border đồng nhất.
...
Empty state và pagination phải dùng component chung. 15. Pill và badge
...
Trạng thái semantic dùng modifier chung. 16. Button và khoảng cách
Các variant chuẩn:
Primary.
Secondary.
Danger.
...
Icon-only button phải có aria-label. 17. Modal và pop-up
Modal chuẩn gồm:
Header.
Body có thể scroll.
Footer cố định.
...
window.alert()
window.confirm()
nếu dự án đã có modal hoặc toast chung. 18. Validation
...
Không dùng inline style.
Field error hiển thị cạnh field, không biến thành toast. 19. Toast và feedback thao tác
...
Validation của field vẫn hiển thị cạnh field, không chuyển thành toast. 20. Filter, sort và pagination
...
Không tự thêm chức năng hoặc endpoint chưa tồn tại chỉ để lấp đầy menu. 22. CSS architecture và responsive
...
Không xóa class cũ trước khi tìm toàn project và xác nhận không còn sử dụng. 23. Accessibility
Giữ hoặc bổ sung:
aria-label
aria-labelledby
aria-describedby
...
Toast hoặc feedback động phải có vùng aria-live phù hợp. 24. Cách chia nhóm UI
...
Không format file không liên quan. 25. Testing và kiểm tra sau khi sửa
...
Nếu không chạy được vì môi trường, database, file lock hoặc dịch vụ ngoài, phải báo chính xác nguyên nhân. 26. Tiêu chí hoàn thành
...
Không tuyên bố hoàn thành quá mức khi còn phần chưa kiểm tra. 27. Báo cáo cuối
