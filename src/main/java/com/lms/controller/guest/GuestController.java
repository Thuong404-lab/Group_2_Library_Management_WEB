...
public class GuestController extends LocalizedControllerSupport {
    ...
    public String viewBookList(@RequestParam(required = false) String keyword,
        ...
        if ("title_asc".equals(sort))
            ...
            sorting = Sort.by("bookId").ascending();
        else
    ...
    public String aboutPage(Model model) {
        ...
        try {
            model.addAttribute("borrowFeePerBook",
                    Long.parseLong(settings.getOrDefault("Borrow_Fee_Per_Book", "5000")));
        } catch (NumberFormatException e) {
        ...
        try {
            model.addAttribute("damageCompensationAmount",
                    Long.parseLong(settings.getOrDefault("Damage_Compensation_Amount", "120000")));
        } catch (NumberFormatException e) {
